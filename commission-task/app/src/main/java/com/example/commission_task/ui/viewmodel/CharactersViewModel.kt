...
import com.example.commission_task.data.CharactersRepository
...
class CharactersViewModel(
    ...
    private fun updateGroupedCharacters() {
        val grouped = visibleCharacters
            .groupBy { character ->
                if (character.name.isNotEmpty())
                    character.name.first().uppercaseChar()
                else
                    '#'
            }
            .toSortedMap()
        uiState = uiState.copy(groupedCharacters = grouped)
    }
